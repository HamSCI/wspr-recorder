...
class WsprRecorder:
    ...
    async def run(self) -> None:
        ...
        # Make the decode mode LOUD at startup, and name the cause when it
        # is off — a silently-disabled SpotSink is exactly how a receiver
        # ends up RECORDING WAVs but never DECODING.  Never let that be
        # silent, and never blame the wrong knob (2026-09-10).
        level, verdict = self.spot_sink.startup_verdict()
        getattr(logger, level)(verdict)
